...
description: 研发需求智能评审与治理技能。当用户需要提交研发需求、评估需求优先级、进行需求评审打分、区分战略型/经营优化型/日常改进型/无效需求时触发。自动依据公司评分规则（业务价值、战略匹配度、影响范围、投入产出比、紧急程度、实施复杂度）对需求进行多维度打分、定级分类（A/B/C/D四级）、生成提报材料和部门宣导文案。支持自然语言输入，AI 自动提取关键信息并评分。融入RICE/JTBD/OST/Amazon Working Backwards/OKR/North Star等专业PM框架进行深度分析。具备自学习能力，可根据用户纠正自动校准评分策略。Make sure to use this skill whenever the user mentions 需求评审、需求打分、需求定级、需求优先级、研发需求、需求治理、demand review、需求提报、需求评估、需求分类、需求审批、需求立项 or wants to automatically score, classify, or generate submission materials for R&D demands.
---
...
**在评分前，必须先运用PM专业框架对需求进行深度分析。** 读取 [references/pm_frameworks.md](references/pm_frameworks.md) 获取框架详细说明。
...
3. **PM框架必做**：评分前必须完成PM框架分析，框架分析结论直接影响评分
4. **框架交叉验证**：当多个框架指向一致结论时，评分置信度更高
5. **语义理解**：自然语言描述可能不完整，AI 需要理解上下文进行合理推断
6. **否定词识别**：注意"无"、"不"、"仅个人"等否定/低价值词汇
7. **量化优先**：优先使用用户提供的量化数据（如"提升30%"、"1人月"）
8. **透明解释**：每个维度的评分必须有明确的理由说明，包含PM框架分析依据
9. **主动收集反馈**：评分后主动询问用户是否认可，推动自学习循环
10. **RICE量化**：尽可能将模糊描述转化为RICE可量化指标
