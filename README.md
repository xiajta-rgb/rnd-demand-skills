...
→ 6维度评分 → 评分校验 → 结构化评分理由(判断依据+扣分原因) → AI建议(8维度)
...
- **去重检查**：关键词重叠≥60%自动提示相似需求，已删除需求不参与
- **PM框架评分**：8大PM框架交叉验证，6维度100分制
- **评分校验**：validate_score 校验总分/等级/维度分数一致性
- **评分置信度**：信息充分=高，部分推测=中，大量假设=低
- **结构化评分理由**：每个维度拆分为"判断依据+扣分原因"
- **AI建议**：8维度开发人员视角优化建议
- **自动映射**：评分→等级→标签→优先级→需求类型，一键推导
- **D级驳回理由**：D级需求必须给出具体驳回原因和改进建议
- **需求库管理**：自动编号、状态流转（带校验）、变更历史、软删除、模糊搜索
- **数据安全**：文件锁防并发、原子写入、自动备份
...
AI：[去重检查] → [补全] → [PM框架分析] → 85分 A级战略型(置信度:高) → [AI建议8维度] → 确认入库？
```
...
│   ├── calibration.yaml            # 校准规则（自学习产出，含示例注释）
│   ├── company_context.yaml        # 公司知识记忆（含版本号和更新记录）
│   ├── demand_library.json         # 需求库数据（含变更历史）
│   └── feedback_history.json       # 反馈历史
├── references/
│   ├── pm_frameworks.md            # PM框架评分决策参考
│   └── demand_refinement_guide.md  # 补全方法论与示例（对齐标准字段）
├── scripts/
│   ├── demand_scorer.py            # 评分引擎v7（结构化理由+AI建议+置信度+驳回理由）
│   ├── demand_refiner.py           # 补全引擎v3（对齐标准字段）
│   ├── demand_library.py           # 需求库v5（文件锁+原子写入+备份+软删除+评分校验）
